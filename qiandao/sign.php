...
header("Access-Control-Allow-Origin: *");
		...
		//先判断数据库有没有这个用户的数据
		$sql = "select * from jifen_qiandao where id = '$openid'";//qiandao
		$result = mysqli_query($conn,$sql);
		$num = mysqli_num_rows($result);
		$sql2 = "select * from jifen_user where oppenid = '$openid'";//jifen
		...
		if($num1!=0){
			if($num != 0){
				...
				if($result_update){
					echo "<script language='javascript'>alert('恭喜您，签到成功，请继续坚持！')<script>";
				}else{
					echo "签到失败！";
				}
			}else{
			...
			echo "<script>alert('请先完善个人信息！')；history.go(-1);</script>";
		}
?>	
