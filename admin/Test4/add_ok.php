...
if($_POST['action'] == "add"){
	if(!($_POST['add_item'] && $_POST['remark'])){
		echo "输入不允许为空。点击<a href='javascript:onclick=history.go(-1)' style='color:red font-size:16px'>这里</a>返回";
	}else{
		...
		$num = $_POST['total_intergral']+$_POST['add_item'];
		$sqlstr = "update jifen_user set  total_intergral = '".$num."' where userid = ".$_POST['userid'];//定义更新语句
